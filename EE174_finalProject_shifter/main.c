...
int main(void) {
	...
	while(1) {
		barFormatter(findMaxADC(), SOUND);
		barFormatter(g_adcInfrared, INFRA);
	    __bis_SR_register(LPM0_bits + GIE);
	}
}
...
//	percentage = (temp/4095);
	...
	if (type == SOUND)
		shiftByteOut(barByte, SOUND);
	else
		shiftByteOut(barByte, INFRA);
